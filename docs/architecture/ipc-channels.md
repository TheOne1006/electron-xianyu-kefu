...
│                        渲染进程 (React)                               │
│                                                                       │
│ config.get/save  browser.launch  prompts.*  product.*  conversation.* │
└────────┬───────────────┬───────────────┬───────────┬──────────┬──────┘
         │               │               │           │           │
    ┌────▼───────────────▼───────────────▼───────────▼───────────▼────┐
    ...
    └─────────────────────────────────────────────────────────────────┘
         ▲
         │  reply-queue:peek / reply-queue:dequeue
...
| 通道             | 方向            | 请求参数    | 响应类型 `IpcResult<T>` | 说明               |
| ---------------- | --------------- | ----------- | ----------------------- | ------------------ |
| `browser:launch` | renderer → main | `AppConfig` | `T = null`              | 启动闲鱼浏览器窗口 |

### Conversation（对话历史）

| 通道                  | 方向            | 请求参数            | 响应类型 `IpcResult<T>` | 说明                                        |
| --------------------- | --------------- | ------------------- | ----------------------- | ------------------------------------------- |
| `conversation:upsert` | injected → main | `ConversationInput` | `T = null`              | 推送新消息到 Agent 流水线（创建或更新对话） |

### Reply Queue（回复队列）

| 通道                  | 方向                       | 请求参数                            | 响应类型 `IpcResult<T>`             | 说明                   |
| --------------------- | -------------------------- | ----------------------------------- | ----------------------------------- | ---------------------- |
| `reply-queue:peek`    | injected → main            | 无                                  | `T = {chatInfo, replyText} \| null` | 拉取待发送的 AI 回复   |
| `reply-queue:dequeue` | injected → main            | `{chatId: string}`                  | `T = null`                          | 确认回复已发送，出队   |
| `reply-queue:enqueue` | renderer → main → injected | `{chatId: string, content: string}` | `T = {success: boolean}`            | 下发回复到浏览器注入层 |
...
| `conversation:upsert`  | renderer / injected → main | `ConversationInput` | `T = ConversationInput`         | 创建或更新对话 |
...
domain  = 业务域（config / browser / agent-config / product / conversation / reply-queue）
action  = 操作（get / save / list / getById / create / upsert / deleteById / delete / peek / enqueue / dequeue / launch / ...）
```
